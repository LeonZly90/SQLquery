...
select #wrongData.*, SubDivCode.DisplayName as SubDiv_name,SubCode into #wrongData_cal 
from #wrongData 
...
select ParentCode,SubCode,Obs_SubDiv,TradeNo,Contractor_Display, count(projectID)as Obs_Projects_count,projectID, Project_No,Project_Name,Project_Company,Obs_id,SubcontractorId
into #scp 
from(
	select --ParentCode,
	ParentCode,SubCode,Obs_SubDiv,c.Display_Name as Contractor_Display,c.TradeNo,dbo.Project.id as projectID ,Project_No,Project_Name,Project_Company
	,obsTemp.id as Obs_id,c.id as SubcontractorId --select * 
	from #update_ObsSubDivCode obsTemp
	join SubDivCode s on s.DisplayName = obsTemp.Obs_SubDiv
	join dbo.Project on obsTemp.Obs_Proj_Id = Project.id
	join dbo.Contractor c on c.id = obsTemp.Obs_Contractor 
	join dbo.WeightOfIssue w on obsTemp.Obs_Weighted = w.id --order by obsTemp.id
	--join dbo.ProjectSubcontractor p on p.SubcontractorId = c.id
	where Obs_SubDiv is not null and Obs_SubDiv !='_Select Subcode' 
	and Obs_date >= '2020-01-01' and Obs_Compliance_Date <= '2022-01-01' --1/25/22
	and ParentCode = '05' and SubCode = '20' and Project_Company='PCC'
	--NOW 3687 without wrong data--4208 rows/528 rows not correct. was 1798 1/25/22
	) a 
group by projectID,SubCode,SubcontractorId,ParentCode,Obs_SubDiv,TradeNo,Contractor_Display,Project_No,Project_Name,Project_Company,Obs_id --1430 rows unique for each trade/parent code
...
from(
	...
	where Obs_SubDiv is not null and Obs_SubDiv !='_Select Subcode' 
	and Obs_date >= '2020-01-01' and Obs_Compliance_Date <= '2022-01-01' --1/25/22
	and ParentCode = '05' and SubCode = '20' and Obs_status = 'No') x
...
from(
	...
	where Obs_SubDiv is not null and Obs_SubDiv !='_Select Subcode' 
	and Obs_date >= '2020-01-01' and Obs_Compliance_Date <= '2022-01-01' --1/25/22
	and ParentCode = '05' and SubCode = '20' 
	and WeightedValue=0

	--from dbo.Observation obs
	--join dbo.Project on obs.Obs_Proj_Id = Project.id
	--join dbo.WeightOfIssue w on obs.Obs_Weighted = w.id
	--where WeightedValue=0
	) x
...
from(
	...
	where Obs_SubDiv is not null and Obs_SubDiv !='_Select Subcode' 
	and Obs_date >= '2020-01-01' and Obs_Compliance_Date <= '2022-01-01' --1/25/22
	and ParentCode = '05' and SubCode = '20' and WeightedValue=1) x --593 rows
group by id, Project_No,Project_Name
...
from(
	...
	where Obs_SubDiv is not null and Obs_SubDiv !='_Select Subcode' 
	and Obs_date >= '2020-01-01' and Obs_Compliance_Date <= '2022-01-01' --1/25/22
	and ParentCode = '05' and SubCode = '20' and WeightedValue=2) x --386 rows
group by id, Project_No,Project_Name
...
from(
	...
	where Obs_SubDiv is not null and Obs_SubDiv !='_Select Subcode' 
	and Obs_date >= '2020-01-01' and Obs_Compliance_Date <= '2022-01-01' --1/25/22
	and ParentCode = '05' and SubCode = '20' and WeightedValue=3) x --3686 rows
group by id, Project_No,Project_Name
...
from(
	...
	where Obs_SubDiv is not null and Obs_SubDiv !='_Select Subcode' 
	and Obs_date >= '2020-01-01' and Obs_Compliance_Date <= '2022-01-01' --1/25/22
	and ParentCode = '05' and SubCode = '20' and WeightedValue=4) x --1948 rows
group by id, Project_No,Project_Name
...
from(
	...
	where Obs_SubDiv is not null and Obs_SubDiv !='_Select Subcode' 
	and Obs_date >= '2020-01-01' and Obs_Compliance_Date <= '2022-01-01' --1/25/22
	and ParentCode = '05' and SubCode = '20' and WeightedValue=5) x --2245 rows
group by id, Project_No,Project_Name
...
----------------------------------------------------------------------------------
				...
				select #scp.*,
				severity_0,PCC_SUM_severity_0,SUB_SUM_severity_0,PCC_SUB_SUM_severity_0,days_from_today_0,
				severity_1,PCC_SUM_severity_1,SUB_SUM_severity_1,PCC_SUB_SUM_severity_1,days_from_today_1,
				severity_2,PCC_SUM_severity_2,SUB_SUM_severity_2,PCC_SUB_SUM_severity_2,days_from_today_2,
				severity_3,PCC_SUM_severity_3,SUB_SUM_severity_3,PCC_SUB_SUM_severity_3,days_from_today_3,
				severity_4,PCC_SUM_severity_4,SUB_SUM_severity_4,PCC_SUB_SUM_severity_4,days_from_today_4,
				severity_5,PCC_SUM_severity_5,SUB_SUM_severity_5,PCC_SUB_SUM_severity_5,days_from_today_5
				into #scpi 
				from #scp --1430
...
from(
	...
	where Obs_SubDiv is not null and Obs_SubDiv !='_Select Subcode' 
	and Obs_date >= '2020-01-01' and Obs_Compliance_Date <= '2022-01-01' --1/25/22
	and ParentCode = '05' and SubCode = '20' and Obs_Repeated='Yes') x --867
group by id, Project_No,Project_Name
...
from(
	...
	where Obs_SubDiv is not null and Obs_SubDiv !='_Select Subcode' 
	and Obs_date >= '2020-01-01' and Obs_Compliance_Date <= '2022-01-01' --1/25/22
	and ParentCode = '05' and SubCode = '20' and Obs_PriorTalk=1) x --867
...
from(
	...
	where Obs_SubDiv is not null and Obs_SubDiv !='_Select Subcode' 
	and Obs_date >= '2020-01-01' and Obs_Compliance_Date <= '2022-01-01' --1/25/22
	and ParentCode = '05' and SubCode = '20' and Obs_Foreman_Present=0) x --2946
...
stuff((SELECT distinct ', ' + #scpirtf.Contractor_Display from #scpirtf 
		where ParentCode = @Div and SubCode = @SubDiv
		FOR XML PATH('')),1,1,''
		) Contractor_List
into #Contractor_List 
from #scpirtf where ParentCode = @Div and SubCode = @SubDiv;
--select * from  #Contractor_List 

--part 8.1 delete duplicate records because of left join
with CTE as 
(
select *, ROW_NUMBER() over (partition by Project_No order by Project_No) as RN
from #scpirtf
)
...
isnull(count(distinct Project_No),0) as Project_No_count,
--count(Project_Name) as Project_Name_count,
--count(Project_Company) as Project_Company_count,

isnull(sum(PCC_SUM_severity_0),0) as PCC_SUM_severity_0,
isnull(sum(SUB_SUM_severity_0),0) as SUB_SUM_severity_0,
isnull(sum(PCC_SUB_SUM_severity_0),0) as PCC_SUB_SUM_severity_0,
--avg(days_from_today_0) as avg_days_from_today_0,

isnull(sum(PCC_SUM_severity_1),0) as PCC_SUM_severity_1,
isnull(sum(SUB_SUM_severity_1),0) as SUB_SUM_severity_1,
isnull(sum(PCC_SUB_SUM_severity_1),0) as PCC_SUB_SUM_severity_1,
--avg(days_from_today_1) as avg_days_from_today_1,

isnull(sum(PCC_SUM_severity_2),0) as PCC_SUM_severity_2,
isnull(sum(SUB_SUM_severity_2),0) as SUB_SUM_severity_2,
isnull(sum(PCC_SUB_SUM_severity_2),0) as PCC_SUB_SUM_severity_2,
--avg(days_from_today_2) as avg_days_from_today_2,

isnull(sum(PCC_SUM_severity_3),0) as PCC_SUM_severity_3,
isnull(sum(SUB_SUM_severity_3),0) as SUB_SUM_severity_3,
isnull(sum(PCC_SUB_SUM_severity_3),0) as PCC_SUB_SUM_severity_3,
--avg(days_from_today_3) as avg_days_from_today_3,

isnull(sum(PCC_SUM_severity_4),0) as PCC_SUM_severity_4,
isnull(sum(SUB_SUM_severity_4),0) as SUB_SUM_severity_4,
isnull(sum(PCC_SUB_SUM_severity_4),0) as PCC_SUB_SUM_severity_4,
--avg(days_from_today_4) as avg_days_from_today_4,

isnull(sum(PCC_SUM_severity_5),0) as PCC_SUM_severity_5,
isnull(sum(SUB_SUM_severity_5),0) as SUB_SUM_severity_5,
isnull(sum(PCC_SUB_SUM_severity_5),0) as PCC_SUB_SUM_severity_5,
--avg(days_from_today_5) as avg_days_from_today_5,

isnull(sum(Items_Repeated),0) as Items_Repeated_sum,
isnull(sum(PCC_SUM_repeat),0) as PCC_SUM_repeat,
isnull(sum(SUB_SUM_repeat),0) as SUB_SUM_repeat,
isnull(sum(PCC_SUB_SUM_repeat),0) as PCC_SUB_SUM_repeat,

isnull(sum(Obs_PriorTalk_count),0) as Obs_PriorTalk_sum,
isnull(sum(PCC_SUM_PriorTalk),0) as PCC_SUM_PriorTalk,
isnull(sum(SUB_SUM_PriorTalk),0) as SUB_SUM_PriorTalk,
isnull(sum(PCC_SUB_SUM_PriorTalk),0) as PCC_SUB_SUM_PriorTalk,

isnull(sum(Foreman_NOT_Present_count),0) as Foreman_NOT_Present_sum,
isnull(sum(PCC_SUM_Foreman),0) as PCC_SUM_Foreman,
isnull(sum(SUB_SUM_Foreman),0) as SUB_SUM_Foreman,
isnull(sum(PCC_SUB_SUM_Foreman),0) as PCC_SUB_SUM_Foreman
--into #except_rank_part
from #scpirtf join #Contractor_List on #scpirtf.Obs_SubDiv = #Contractor_List.Obs_SubDiv
--where #scpirtf.Obs_SubDiv like '%05-20%' 
group by ParentCode, Contractor_List, #scpirtf.Obs_SubDiv
order by #scpirtf.Obs_SubDiv
-----------------------------------------------------------
--select distinct Contractor_Display-- into #contractorList_only 
--from #scpirtf --where #scpirtf.Obs_SubDiv like '%05-20%' 

--select ParentCode, Obs_SubDiv, TradeNo, Contractor_Display,
...
--from #scpirtf
--group by ParentCode, #scpirtf.Obs_SubDiv, TradeNo, Contractor_Display --Contractor_List,
--order by Contractor_Display
...
drop table #sever_0 --select * from #sever_3        select * from #scp
drop table #sever_1
drop table #sever_2
drop table #sever_3
drop table #sever_4
...
--exec dbo.[DrillDownReport_scpirtf] '09', '20','2020-01-01','2022-01-01'
--exec dbo.[DrillDownReport_2] '05', '20','2020-01-01','2022-01-01'
--exec [dbo].[DrillDownReport_rank] '05', '20','2020-01-01','2022-01-01','rock','denk','','',''
--select *
----distinct SubcontractorId 
--from ProjectSubcontractor
--select * from Contractor order by id