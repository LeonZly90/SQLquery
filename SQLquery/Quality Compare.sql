...
--EXEC dbo.[RepeatSummaryReport]'1601712'

----select max (Obs_Date) from Observation
--select * from PreinstallationChecklist
--select * from ProjectCheckList
...
------
--Last_Review_Date 223, now 236 for multi
...
select 	distinct Project_No, ProjectNoName, Company--,
		--case 
...
from(
	select Project_No, Project_No+' '+Project_Name as ProjectNoName, --Project.Project_Company,
...
from(
	...
	[% Pre-install Complete],[Pre-install not Completed to Date], Company, Project_No--, TradeName
	from #Items_Total 
...
-----------
declare @ProjectID_or_Name1 nvarchar(100), @Company1 nvarchar(10)
set @ProjectID_or_Name1 = '1501470'--'1602130'
set @Company1 = 'PCC'

declare @ProjectID_or_Name2 nvarchar(100), @Company2 nvarchar(10)
set @ProjectID_or_Name2 = '1800179'
set @Company2 = 'PCCI'

select * 
--into #xx
from #All_Compare_Project 
where 
(Project_No = @ProjectID_or_Name1 or ProjectNoName like  '%'+(@ProjectID_or_Name1)+'%') and Company = @Company1
or
(Project_No = @ProjectID_or_Name2 or ProjectNoName like  '%'+(@ProjectID_or_Name2)+'%') and Company = @Company2
...
pivot
(count(a.Company)
  for a.ProjectNoName in ([1602130 810 N Clark - Cedar Street Capital],[1800179 Verus Partners - Greenwood Business Center])
) as piv



with cte as (
  select #xx.ProjectNoName, v.subject, v.value
  from #xx
  cross apply (values ('[1602130 810 N Clark - Cedar Street Capital]','[1602130 810 N Clark - Cedar Street Capital]'),('[1800179 Verus Partners - Greenwood Business Center]','[1800179 Verus Partners - Greenwood Business Center]')) v(subject,value)
)
select subject, 
ProjectNoName, Company,Last_Date_of_Job_Review,total_items,Percent_Closed,
Items_Repeated,severity_0,severity_1,severity_2,severity_3,severity_4,severity_5,
Total_Project_Cost,Project_Cost_to_Date,Percent_Project_complete,Pepper_Cost_Quailty,Sub_Cost_Quailty,Cost_of_Quality_Percent,
[1-15 days],[16-30 days],[31-60 days],[61-90 days],[>90 days],
[% Pre-install Complete],[Pre-install not Completed to Date] 

from cte
pivot (max(value) for [ProjectNoName] in (
ProjectNoName, Company,Last_Date_of_Job_Review,total_items,Percent_Closed,
Items_Repeated,severity_0,severity_1,severity_2,severity_3,severity_4,severity_5,
Total_Project_Cost,Project_Cost_to_Date,Percent_Project_complete,Pepper_Cost_Quailty,Sub_Cost_Quailty,Cost_of_Quality_Percent,
[1-15 days],[16-30 days],[31-60 days],[61-90 days],[>90 days],
...
declare @ProjectID_or_Name nvarchar(100)
set @ProjectID_or_Name values '1602130','1800179'
select * from  #All_Compare_Project where Project_No in (@ProjectID_or_Name)

execute [dbo].[ComparisonReport] '05', '2001840', 'PCC', '1700839', 'PCC','1402148','PCC','1501256','PCC','1500998','PCC'
